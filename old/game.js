...
for(let i=1;i<snake.length;i++){
    ...
    if(dead === 1){
            ...
            window.location.href = "Snake.php?score=" + count;
   ...
   // console.log(snake[1].x);
    ...
    ctx.font = "15px Arial";
ctx.fillStyle="#ffccff";
ctx.fillText(count,(window.innerWidth-105),40);
...
ctx.fillText(third,20,85);
    ...
    if(Math.abs(snake[0].x-food.x) < 1.0 && Math.abs((snake[0].y-food.y)) < 1.0){
        count+=100;
        foodpos();
        let snakeNew={
            x:0,
